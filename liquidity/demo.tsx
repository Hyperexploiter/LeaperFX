...
import { ArrowRight, CheckCircle, Clock, DollarSign, TrendingUp, Shield, Zap, AlertTriangle, Activity, BarChart3, Settings, Layers } from 'lucide-react';
...
const DeFiFXDemo = () => {
  ...
  const simulateTransaction = () => {
    ...
    const inventoryGap = Math.max(0, requiredAmount - storeInventory[selectedCurrency]);
    ...
    const steps = [
      ...
      { id: 4, text: `Inventory check: Need ${requiredAmount.toLocaleString()} ${selectedCurrency}, have ${storeInventory[selectedCurrency].toLocaleString()}`, status: "pending", delay: 2000 },
            ...
            <div>
                ...
                <div className="flex">
                  ...
                  <div className="ml-3">
                    <p className="text-sm text-yellow-700">
                      <strong>Inventory Gap:</strong> Store has {storeInventory[selectedCurrency].toLocaleString()} {selectedCurrency},
                      but you need {Math.round(orderAmount * exchangeRates[selectedCurrency]).toLocaleString()} {selectedCurrency}.
                      <br />
                      <strong>Shortfall:</strong> {Math.max(0, Math.round(orderAmount * exchangeRates[selectedCurrency]) - storeInventory[selectedCurrency]).toLocaleString()} {selectedCurrency}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                ...
                <div className="text-sm text-blue-700 space-y-1">
                  <div>• Flash loan: ${Math.round(Math.max(0, Math.round(orderAmount * exchangeRates[selectedCurrency]) - storeInventory[selectedCurrency]) / exchangeRates[selectedCurrency] * 1.1)} USDC</div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {Object.entries(storeInventory).map(([currency, value]) => {
                      const numericAmount = value as number; // Assert value is a number
